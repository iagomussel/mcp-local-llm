...
export class HumanizeContentTool extends BaseTool {
  getToolDefinition() {
    return {
      ...
      inputSchema: {
        ...
        properties: {
          content: {
            type: 'string',
            description: 'The content to humanize - can be text, URL, or file path',
          },
        },
        required: ['content'],
      },
  ...
  async handle(args) {
    ...
    // Auto-select model and parameters for humanization
    const model = await this.selectBestModel(textToHumanize);
    const temperature = 0.85; // Higher temperature for more creative humanization
